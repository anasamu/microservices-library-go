...
import (
	...
	"github.com/anasamu/microservices-library-go/libs/database/providers/mongodb"
	"github.com/anasamu/microservices-library-go/libs/database/providers/mysql"
	"github.com/anasamu/microservices-library-go/libs/database/providers/postgresql"
	"github.com/anasamu/microservices-library-go/libs/database/providers/redis"
...
func registerProviders(databaseManager *DatabaseManager, logger *logrus.Logger) {
...
func examplePostgreSQLConnection(databaseManager *DatabaseManager) {
...
func exampleMongoDBConnection(databaseManager *DatabaseManager) {
...
func exampleRedisConnection(databaseManager *DatabaseManager) {
...
func exampleMySQLConnection(databaseManager *DatabaseManager) {
...
func exampleQueryOperations(databaseManager *DatabaseManager) {
...
func exampleTransactionOperations(databaseManager *DatabaseManager) {
	...
	if databaseManager.IsProviderConnected("postgresql") {
		...
		// Execute transaction
		err := databaseManager.WithTransaction(ctx, "postgresql", func(tx Transaction) error {
...
func exampleHealthChecks(databaseManager *DatabaseManager) {
...
func exampleDatabaseStats(databaseManager *DatabaseManager) {
...
// ExamplePreparedStatements demonstrates prepared statements
func ExamplePreparedStatements() {
	fmt.Println("\n=== Prepared Statements Example ===")

	// Create logger and database manager
	logger := logrus.New()
	config := DefaultManagerConfig()
	databaseManager := NewDatabaseManager(config, logger)
	...
	// Prepare a statement
	stmt, err := databaseManager.Prepare(ctx, "postgresql", "SELECT id, name, email FROM users WHERE id = $1")
	if err != nil {
	...
	var name, email string
	if err := row.Scan(&id, &name, &email); err != nil {
		log.Printf("Failed to scan prepared statement result: %v", err)
		return
	}

	fmt.Printf("Prepared statement result: ID: %d, Name: %s, Email: %s\n", id, name, email)
}
...
func ExampleProviderCapabilities() {
	...
	providers := databaseManager.GetSupportedProviders()
	for _, providerName := range providers {
		features, connInfo, err := databaseManager.GetProviderCapabilities(providerName)
		if err != nil {
...
func ExampleConnectionManagement() {
	...
	providers := []string{"postgresql", "redis", "mongodb"}
	for _, providerName := range providers {
	...
	fmt.Printf("Connected providers: %v\n", databaseManager.GetConnectedProviders())

	// Disconnect from providers
	for _, providerName := range providers {
		if err := databaseManager.Disconnect(ctx, providerName); err != nil {
	...
	if err := databaseManager.Close(); err != nil {
		...
		fmt.Println("Database manager closed successfully")
	}
}
